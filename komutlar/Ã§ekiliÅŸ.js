...
if(!args[0]) return message.channel.send(`❌ Bir zaman belirtmelisin!`)
        ...
        message.channel.send(`✔️ Çekiliş ${channel} kanalında başarıyla başladı!`)
        let Embed = new MessageEmbed()
        .setTitle(`<a:tadas:812332337705648138> Çekiliş! <a:tadas:812332337705648138>`)
        ...
        setTimeout(() => {
            if(m.reactions.cache.get("🎉").count<=1){
                message.channel.send(`<a:tadas:812332337705648138> Çekilişe katılan kişi sayısı: ${m.reactions.cache.get("🎉").count}`)
                return message.channel.send(`❌ Çekilişe yeterli kişi katılmadı!`)
            }
            
            let winner = m.reactions.cache.get("🎉").users.cache.filter(u=>!u.bot).random()
            channel.send(`<a:tadas:812332337705648138> **${prize}** çekilişinin kazananı -> ${winner}`)
        }, ms(args[0]));
