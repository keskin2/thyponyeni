...
client.channels.cache.get(kanal).send(` <a:ggf:812969377594343434> ${member} Adlı Kullanıcı Aramıza Katıldı. Kullanıcıyı Davet Eden  ${daveteden}  Toplam **${davetsayi}** Daveti Oldu`)  
...
client.on("guildMemberRemove", async member => {
  ...
  if (!davetçi) {
    return client.channels.cache.get(kanal).send(`<a:kgf:812969072341680148> ${member} Adlı Kullanıcı Aramızdan Ayarıldı Davet Eden Bulunamadı!`);
  } else {
     
client.channels.cache.get(kanal).send(`<a:kgf:812969072341680148> ${member} Adlı Kullanıcı Aramızadan Ayrıldı Kullanıcıyı Davet Eden ${daveteden}  Toplam  **${davetsayi}** Daveti Kaldı`)  
...
client.on("guildMemberAdd", member => {
  ...
  if (ayyy < 1) {
    kontrol = "**Şüpheli** <a:dikkat:808028216161599488>";
  }
  if (ayyy > 1) {
    kontrol = "**Güvenilir** <a:guvenlik:808254179255648256>";
  }
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .setDescription(
      `<a:bt:810112130929524737> **Hoşgeldin!** ${
        member.user
      }, seninle beraber **${
        guild.memberCount
      }** kişi olduk! \n <a:bt:810112130929524737> Kaydının yapılması için  **İsim** ve **Yaş** Yazman Gerek. \n <a:YklenmeGif:753288640121012244> Hesap Kuruluş: **${moment(
        user.createdAt
      ).format("DD")} ${aylar[moment(user.createdAt).format("MM")]} ${moment(
        user.createdAt
      ).format(
        "YYYY HH:mm:ss"
       )}** \n <a:yukleme:810111973474959401> Bu vatandaş: ${kontrol} \n <a:rainbowok:794869466768080896> <@&${kayıtçı}> Rolundeki Yetkililer Sizinle İlgilecektir
...
////.setTitle('Bir Gold Üye Belirdi! <a:dansedenkedi:773868808322547743>')
.setDescription('<a:dansedenkedi:773868808322547743> Aleyküm Selam. Hoş Geldin! <a:hypeshny:810112127952093225>')
.setTimestamp()
.setFooter('Thyphon')
.setColor(0x36393E)
...
client.on("message", async msg => {
  ...
  db.set(`goldzzz_${msg.author.id}`, Date.now());
    ...
    .setDescription(` İşte benim **Sahibim geldi Aç Yolu**! <@${msg.author.id}>`)
    .setColor("BLUE")
  ...
  client.on('message', msg => {
    if (msg.content === 'minnos') {
      msg.channel.send('<a:minnos:810445053910188044>');
    }
  });
  ...
  client.on('message', msg => {
    if (msg.content === 'cool') {
      msg.channel.send('<a:Cool:810445248337674251>');
    }
  });
  ...
  client.on('message', msg => {
    if (msg.content === 'hehe') {
      msg.channel.send('<a:caT:810445311352635453>');
    }
  });
  ...
  client.on('message', msg => {
    if (msg.content === 'havalı') {
      msg.channel.send(' <a:hawali:811570959050997770>');
    }
  });
  ...
  client.on('message', msg => {
    if (msg.content === 'elmas') {
      msg.channel.send('<a:B_Elmas:811230910938349578>');
    }
  });
  ...
  client.on('message', msg => {
    if (msg.content === 'ateş') {
      msg.channel.send('<a:ates:811571167227936799>');
    }
  });
  ...
  client.on('message', msg => {
    if (msg.content === 'kalp') {
      msg.channel.send(' <a:galp:812331849836920842>');
    }
  });
  ...
  client.on('message', msg => {
    if (msg.content === 'minecraft') {
      msg.channel.send('<a:mc:794690993743200316>');
    }
  });
  ...
  client.on("message", async msg => {
    ...
    .setColor("RANDOM")
    .setDescription(" <a:bt:810112130929524737> **Bir Gold Üye Belirdi Hizaya Geçin!** <a:dansedenkedi:773868808322547743> ");
